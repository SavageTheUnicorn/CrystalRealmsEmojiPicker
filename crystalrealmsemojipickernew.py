...
class EmojiPicker:
    ...
    def setup_win32_hotkeys(self):
        ...
        # Callback function for handling hotkeys
        def _hotkey_callback(hotkey_id):
            try:
        ...
        def _wnd_proc(hwnd, msg, wparam, lparam):
            if msg == WM_HOTKEY:
                # Dispatch hotkey callback on main thread
                self.root.after(1, lambda: _hotkey_callback(wparam))
                return 0
        ...
        try:
            # Register the window class with a unique name
    ...
    # Additionally, modify the register_system_wide_hotkeys method to be more robust
    def register_system_wide_hotkeys(self):
        ...
        # Register each emoji hotkey
        for key, emoji_unicode in self.keybind_to_emoji.items():
            vk_code = self.get_vk_code(key)
            if vk_code:
                try:
                    # Try registering with no modifier first
                    result = ctypes.windll.user32.RegisterHotKey(
                        self.hotkey_hwnd,  # Use the dedicated hotkey window
                    ...
                    if result:
                        # Store mapping from ID to emoji
                        self.hotkey_ids[hotkey_id] = emoji_unicode
                        print(f"Registered hotkey for {key} (VK: {vk_code})")
                        hotkey_id += 1  # Increment ID for next hotkey
                    else:
                        print(f"Failed to register hotkey for {key}")
                except Exception as e:
    ...
    def setup_gui(self):
        ...
        self.controls_label = Label(
            self.container,
            text="Click Once to Select • Click Again to Copy/Send • Right Click for Options • Don't Spam Hotkeys",
            font=("Arial", 10),
            pady=5
        )
